...
typedef struct {
  int flag;           //	フィルタのフラグ
                      //	FILTER_PROC_INFO_FLAG_INVERT_FIELD_ORDER	: フィールドオーダーを標準と逆に扱う (
                      // 標準はボトム->トップになっています ) 	FILTER_PROC_INFO_FLAG_INVERT_INTERLACE		:
...
typedef struct {
  int flag;           //	フィルタのフラグ
                      ...
                      // func_WndProc()にWM_FILTER_MAIN_???のメッセージを送るようにします
  ...
  int *check_default;     //	チェックボックスの初期値郡へのポインタ(チェックボックス数が0ならNULLでよい)
                          //	初期値がマイナス値の場合はボタンになります。ボタンを押したときにWM_COMMAND( WPARAM =
                          // MID_FILTER_BUTTON + n )のウィンドウメッセージが送られます
